 ...
 **/
public class LiteResizeTextureView extends TextureView {

    public LiteResizeTextureView(Context context) {
        super(context);
        currentVideoHeight = 0;
        currentVideoWidth = 0;
    }

    public LiteResizeTextureView(Context context, AttributeSet attrs) {
        super(context, attrs);
        currentVideoHeight = 0;
        currentVideoWidth = 0;
    }
    ...
    @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (JZVideoPlayer.VIDEO_IMAGE_DISPLAY_TYPE
                == JZVideoPlayer.VIDEO_IMAGE_DISPLAY_TYPE_FILL_PARENT) {
        ...
        if (parentWidth != 0 && parentHeight != 0 && videoWidth != 0 && videoHeight != 0) {
            if (JZVideoPlayer.VIDEO_IMAGE_DISPLAY_TYPE
                == JZVideoPlayer.VIDEO_IMAGE_DISPLAY_TYPE_ORIGINAL) {
                // 原图
                height = videoHeight;
                width = videoWidth;
            } else if (JZVideoPlayer.VIDEO_IMAGE_DISPLAY_TYPE
                == JZVideoPlayer.VIDEO_IMAGE_DISPLAY_TYPE_FILL_SCROP) {
                if (viewRotation == 90 || viewRotation == 270) {
                    int tempSize = parentWidth;
                    parentWidth = parentHeight;
                    parentHeight = tempSize;
                ...
                if (((double) videoHeight / videoWidth) > ((double) parentHeight / parentWidth)) {
                    ...
                    width = parentWidth;
                } else if (((double) videoHeight / videoWidth) < ((double) parentHeight
                    / parentWidth)) {
    ...
    public void setVideoSize(int currentVideoWidth, int currentVideoHeight) {
        if (this.currentVideoWidth != currentVideoWidth
